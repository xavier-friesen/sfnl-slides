...
`assets/documenten/voorbeeld/`, als kale fragmenten.
...
Ze zijn niet in één keer goed geweest, en de fouten staan in `documenten-vormentaal.md` omdat ze
...
- Alle vier de pagina's hadden een gat in het midden van 224 tot 372 px, van
  ...
  zichtbare defect van het hele document en het is de reden dat `qa_document.py` nu `gat` meet.
...
- De infographic op pagina 3 voerde zijn eigen lettermaten in — 12, 13 en 15 px binnen de SVG,
  ...
  als de pagina, dus hij hoort dezelfde ladder te gebruiken. `qa_document.py` telde acht maten.
...
Ze zijn opnieuw te maken uit `assets/documenten/voorbeeld/`:

```bash
cp assets/documenten/voorbeeld/*.dc.html <werkmap>/
python scripts/documenten/bouw.py <werkmap> --uit wie-betaalt-de-preventie.html
python scripts/documenten/render.py <werkmap>/wie-betaalt-de-preventie.html
python scripts/documenten/qa_document.py <werkmap>/wie-betaalt-de-preventie.html
```
